...
- **Shadow-mode observer for the upcoming v2.0.0 sensor-map architecture.** Ships OFF by default. When enabled via the environment variable `SENSOR_MAP_V2=1`, a sensor-map layer runs in parallel to the v1.6.0 code path and logs any divergence between the two pipelines to Homebridge's log. The v1.6.0 code path still owns every accessory-registration decision; nothing on disk is modified. Purpose: real-world validation of the v2.0.0 code load path before v2.0.0 GA promotes the flag to on-by-default. See CHANGELOG on the `@beta` channel for the fuller v2.0.0 preview.

### Not changing

- No configuration UI change. The shadow-mode flag is not exposed in the plugin's config form — opt-in requires an explicit environment variable.
- No accessory-registration path change. Users who don't set the env var see byte-identical behavior to v1.6.1.

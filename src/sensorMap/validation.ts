...
export function validateOverride(
  ...
  if (override.stationMac !== undefined) {
    if (typeof override.stationMac !== 'string' || !STATION_MAC_REGEX.test(override.stationMac)) {
      return err(
        `stationMac '${override.stationMac}' is not a MAC address. Use the station picker.`,
        warnings,
      );
    }
  }
  ...
  if ('wrapperId' in override) {
    return err(
      `wrapperId is not a valid override field on ${override.dataPoint}.`,
      warnings,
    );
  }
  ...
  if (isCustom) {
    ...
    if (!override.kind || override.kind === 'unrecognized') {
      return err(
        `Custom dataPoint '${override.dataPoint}' requires 'kind'.`,
        warnings,
      );
    }
    if (!override.measurement) {
      return err(
        `Custom dataPoint '${override.dataPoint}' requires 'measurement'.`,
        warnings,
      );
    }
    ...
    if (!isCompatibleKind(override.measurement, override.kind)) {
      return err(
        `kind '${override.kind}' is not compatible with measurement '${override.measurement}' on ${override.dataPoint}.`,
        warnings,
      );
    }
    ...
    if (override.measurement === 'boolean') {
      ...
      if (override.sourceUnit !== undefined && override.sourceUnit !== 'ms') {
        return err(
          `sourceUnit on timestamp row ${override.dataPoint} must be 'ms'.`,
          warnings,
        );
      }
      if (override.displayUnit !== undefined) {
      ...
      if (!override.sourceUnit) {
        return err(
          `Custom numeric dataPoint '${override.dataPoint}' requires 'sourceUnit'.`,
          warnings,
        );
      }
      ...
      if (!legalSrc.includes(override.sourceUnit)) {
        return err(
          `sourceUnit '${override.sourceUnit}' is not legal for measurement '${override.measurement}'.`,
          warnings,
        );
      }
      if (override.displayUnit !== undefined && !legalSrc.includes(override.displayUnit)) {
        return err(
          `displayUnit '${override.displayUnit}' is not legal for measurement '${override.measurement}'.`,
          warnings,
        );
      }
    }
    ...
    if (override.measurement !== undefined && override.measurement !== defaultRow.measurement) {
      warnings.push(
        `measurement override on known dataPoint '${override.dataPoint}' ignored; measurement is fixed at ${defaultRow.measurement}.`,
      );
    }
    ...
    if (override.kind && override.kind !== 'unrecognized') {
      if (!isCompatibleKind(defaultRow.measurement, override.kind)) {
        return err(
          `kind '${override.kind}' is not compatible with the built-in measurement '${defaultRow.measurement}' on ${override.dataPoint}.`,
          warnings,
        );
      }
    }
    ...
    if (override.sourceUnit !== undefined && override.sourceUnit !== defaultRow.sourceUnit) {
      warnings.push(
        `sourceUnit override on known dataPoint '${override.dataPoint}' ignored; source unit is fixed at ${defaultRow.sourceUnit}.`,
      );
    }
    ...
    if (override.displayUnit !== undefined) {
      ...
      if (!legal.includes(override.displayUnit)) {
        return err(
          `displayUnit '${override.displayUnit}' is not legal for measurement '${defaultRow.measurement}' on ${override.dataPoint}.`,
          warnings,
        );
      }
    }
...
function err(message: string, warnings: string[]): ValidationResult {

...
export function buildEffectiveSensorMap(input: BuildInput): EffectiveSensorMap {
  ...
  const rawMergedByBucket = new Map<string, Record<string, unknown>>();
 ...
 */
export function partitionOverrideLayers(overrides: ReadonlyArray<SensorMapOverride>): {
  global: Map<string, SensorMapOverride>;
  station: Map<string, Map<string, SensorMapOverride>>;
} {
  const global = new Map<string, SensorMapOverride>();
  const station = new Map<string, Map<string, SensorMapOverride>>();
...
interface ResolveInput {
  ...
  /** The config's baseline stamp, for §18.3's default-enabled arithmetic. */
  catalogBaseline: number;
  /** The adopted stamp, gating catalog-versioned wrapper pairs (§19.2). */
  catalogAdopted: number;
  /**
  ...
  /**
   * Called when a CROSS-SCOPE merge hands the row a displayUnit that
   * is illegal for its resolved measurement (PR #66 review F2: a valid
   * global fragment's displayUnit, legal against the identity IT was
   * validated with, meeting a station-authored custom identity). The
   * row loads with the measurement's default display unit instead —
   * an illegal unit must never reach a wrapper, which throws on the
   * first reading.
   */
  onIllegalDisplayUnit: (displayUnit: string, measurement: string) => void;
}
...
function resolveRow(inp: ResolveInput): ResolvedRow {
  const {
    ...
    catalogBaseline, catalogAdopted, onNoWrapper, onWrapperMismatch, onIllegalDisplayUnit,
  } = inp;
  ...
  const legal = LEGAL_UNITS_FOR_MEASUREMENT[measurement as NumericSensorRow['measurement']];

...
export type CatalogStampResult =
  | { status: 'ok'; stamps: CatalogStamps }
 ...
 * closed instead.
 */
export function parseCatalogStamps(config: Record<string, unknown> | undefined): CatalogStampResult {
...
function isStampInteger(v: unknown): v is number {
  return typeof v === 'number' && Number.isInteger(v) && v >= 1;
}

function describeStamp(v: unknown): string {
  if (v === null) {
    return 'null';
  }

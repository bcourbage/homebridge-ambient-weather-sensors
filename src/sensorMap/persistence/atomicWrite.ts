 ...
 */
export async function cleanupStaleTempFiles(dir: string, log: Logger, clock: Clock = REAL_CLOCK): Promise<void> {
  let entries: string[];
  try {
 ...
 */
async function quarantine(filePath: string, log: Logger, clock: Clock): Promise<string> {

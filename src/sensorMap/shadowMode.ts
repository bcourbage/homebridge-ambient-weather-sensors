...
export class ShadowMode {
  ...
  // task #65's flag flip.
  private readonly preservedAccessories = new Map<string, CachedAccessoryShape>();
   ...
   */
  private recoverPreservedAccessories(): void {
    ...
    for (const [uid, accessory] of this.preservedAccessories) {
      ...
      if (result.status === 'inferred') {
        this.log.info(
          `[sensor-map v2 shadow] cache-restore ${uid}: recovered — kind=${result.kind} `
  ...
  onParseTick(input: {
    ...
    if (this.configMode === 'v2') {
      ...
      if (raw !== undefined && !Array.isArray(raw)) {
        this.logDivergenceOnce(
          'sensormap-not-array',
          `v2 sensorMap must be an array; got ${describeType(raw)}. `

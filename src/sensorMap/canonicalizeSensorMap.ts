...
import { buildEffectiveSensorMap, type BuildInput } from './buildEffectiveMap.js';
...
export function canonicalizeSensorMap(input: CanonicalizeInput): SensorMapOverride[] {
  ...
  const defaultsByKey = new Map<string, ConfiguredRow>();
  ...
  const customIdentity = new Map<string, SensorMapOverride>(); // `${MAC}|${dp}` → identity
  ...
  const identityByKey = new Map<string, ConfiguredRow>();
  ...
  // ---- Per-(station, dp) sparse diffs.
  type Diff = Record<string, unknown>;
  const diffsByDp = new Map<string, Map<string, Diff>>(); // dp → stationMac → diff
  for (const row of proposal.rows) {
    if (row.kind === 'unrecognized') {
      continue;
    }
    ...
    for (const f of DIFF_FIELDS) {
      ...
      const proposed = (row as unknown as Record<string, unknown>)[f];
      const base = baseline ? (baseline as unknown as Record<string, unknown>)[f] : undefined;
      if (proposed !== base) {
        diff[f] = proposed;
      }
    }
...
function stableStringify(v: Record<string, unknown>): string {

...
export function canonicalizeSensorMap(input: CanonicalizeInput): SensorMapOverride[] {
  const common: Omit<BuildInput, 'userOverrides'> = {
  ...
  const knownRowFor = (dp: string, ...overrideLayers: ReadonlyArray<unknown>) =>
    defaultRowForConfigOverride(dp, catalogAdopted, ...overrideLayers);
  ...
  const globalAuthorsIdentity = (dp: string): boolean =>
    hasAuthoredIdentity(layers.global.get(dp));
  ...
  const identityFor = (row: ConfiguredRow, stationMac?: string): SensorMapOverride => {
    ...
    if (row.measurement !== 'timestamp' && row.measurement !== 'boolean') {
      identity.sourceUnit = (row as { sourceUnit: SensorUnit }).sourceUnit;
    }
    return identity;
  };
  ...
  const diffRows = (proposed: ConfiguredRow, reference: ConfiguredRow | undefined): Diff => {
    const out: Diff = {};
    for (const f of DIFF_FIELDS) {
      const a = (proposed as unknown as Record<string, unknown>)[f];
      const b = reference ? (reference as unknown as Record<string, unknown>)[f] : undefined;
      if (a !== b) {
        out[f] = a;
      }
    }
  ...
  for (const dp of layers.global.keys()) {
    ...
    if (!row) {
      ...
      const fields: Diff = {};
      for (const f of DIFF_FIELDS) {
        const v = (merged as unknown as Record<string, unknown>)[f];
    ...
    if (isCustom) {
      ...
      if (row.measurement !== 'timestamp' && row.measurement !== 'boolean') {
        fields.sourceUnit = (row as { sourceUnit: SensorUnit }).sourceUnit;
      }
    }
    if (Object.keys(fields).length > 0) {
      entries.push({ dataPoint: dp, fields });
    ...
    for (const dp of perDp.keys()) {
      ...
      if (isCustom) {
        ...
        if (proposed.measurement !== 'timestamp' && proposed.measurement !== 'boolean') {
          fields.sourceUnit = (proposed as { sourceUnit: SensorUnit }).sourceUnit;
        }
      }
  ...
  return entries.map(({ dataPoint, stationMac, fields }) => {
    ...
    return out as unknown as SensorMapOverride;
  });
}

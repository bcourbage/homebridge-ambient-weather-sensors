 ...
 */
const BEAUFORT_BUCKETS_MPH: Array<{ max: number; label: string }> = [
  { max: 1, label: 'Calm' },
...
export function beaufort(speedMph: number): string {
 ...
 */
const RAIN_BUCKETS_IN_HR: Array<{ max: number; label: string }> = [
...
export function rainIntensity(rateInHr: number): string {
 ...
 */
const UV_BUCKETS: Array<{ max: number; label: string }> = [
  { max: 3,  label: 'Low' },        // 0-2
...
export function uvBucket(uv: number): string {
 ...
 */
const COMPASS_16: ReadonlyArray<string> = [
  'N', 'NNE', 'NE', 'ENE',
  'E', 'ESE', 'SE', 'SSE',
  'S', 'SSW', 'SW', 'WSW',
  'W', 'WNW', 'NW', 'NNW',
 ...
 */
export function timeSince(timestampMs: number | undefined, nowMs: number = Date.now()): string {

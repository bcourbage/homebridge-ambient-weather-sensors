...
- Polling updates continue to push values to existing wrappers when they can be identified from cached context, using a "bind existing services / characteristics only" path that NEVER calls `addService`, `removeService`, or attaches missing characteristics on demand. Fresh values from AWN reach `updateCharacteristic` only for accessories whose primary service AND target value characteristic(s) are already attached

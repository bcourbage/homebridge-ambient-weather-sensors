...
export function resolveBatteryField(effectiveMap, stationMac, dataPoint) {
    if (!effectiveMap) {
        return batteryFieldForSensor(dataPoint) ?? null;
    }
    const mac = stationMac.toUpperCase();
    const row = effectiveMap.rows.find(r => r.stationMac.toUpperCase() === mac && r.dataPoint === dataPoint);
    if (!row || row.kind === 'unrecognized') {
        return null;
    }
    return row.hasBatterySubService && row.batteryField ? row.batteryField : null;
}
//# sourceMappingURL=resolveBatteryField.js.map
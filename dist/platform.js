 ...
 */
export function isUnsupportedNewerConfig(config) {
...
export class AmbientWeatherSensorsPlatform {
    constructor(log, config, api) {
        ...
        this.api.on('didFinishLaunching', () => {
            ...
            if (isUnsupportedNewerConfig(this.config)) {
                this.log.error('This configuration was written by plugin version 2.x '
                    + `(configVersion: ${String(this.config.configVersion ?? 'unset')}, `
                    + `sensorMap ${this.config.sensorMap !== undefined ? 'present' : 'absent'}). `
